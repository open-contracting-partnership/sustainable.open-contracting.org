---
permalink: /casos-de-estudio
title: Casos de estudio
description: "En esta sección hemos incluido ejemplos de mejor práctica de implementación APP alrededor del mundo. "
cover: /assets/images/PUBLIC_SPP_background.webp
icon: /assets/images/Notion-others2.svg
notion_id: "1dbcfe1960f3498abe0bdca18d981c41"
sidebar: true
---
{% callout gray /assets/images/Icons_Grey3.svg %}
**En esta sección hemos incluido ejemplos de mejor práctica de implementación APP alrededor del mundo.**
{% endcallout %}

{% database Africa %}
{% gallery large %}

- title: Promoting equal access to government opportunities in Kenya
  link: /casos-de-estudio/promoting-equal-access-to-government-opportunities-in-kenya
  cover: /assets/images/Africa_-_Kenya.webp
  cover_only: true
- title: Communicating SPP operational objectives in an Action Plan in Cape Town
  link: /casos-de-estudio/communicating-spp-operational-objectives-in-an-action-plan-in-cape-town
  cover: /assets/images/Africa_-_Cape_Town.webp
  cover_position: 54.38
  cover_only: true
- title: Using local resources for the design of the first sustainability-accredited building in Ghana
  link: /casos-de-estudio/using-local-resources-for-the-design-of-the-first-sustainability-accredited-building-in-ghana
  cover: /assets/images/Africa_-_Ghana.webp
  cover_position: 54.79
  cover_only: true
- title: Using a two-stage tender to increase collaboration in a construction project in Namibia
  link: /casos-de-estudio/using-a-two-stage-tender-to-increase-collaboration-in-a-construction-project-in-namibia
  cover: /assets/images/Africa_-_Namibia.webp
  cover_position: 54.58
  cover_only: true
{% endgallery %}
{% enddatabase %}

{% database Latin America and the Caribbean %}
{% gallery large %}

- title: Promoting gender inclusion through procurement in Chile
  link: /casos-de-estudio/promoting-gender-inclusion-through-procurement-in-chile
  cover: /assets/images/LAC_-_Chile.webp
  cover_position: 49.5
  cover_only: true
- title: Promoting family agriculture through procurement in Paraguay
  link: /casos-de-estudio/promoting-family-agriculture-through-procurement-in-paraguay
  cover: /assets/images/LAC_-_Paraguay.webp
  cover_position: 53.88
  cover_only: true
- title: Communicating the enabling framework in an SPP Action Plan in Argentina
  link: /casos-de-estudio/communicating-the-enabling-framework-in-an-spp-action-plan-in-argentina
  cover: /assets/images/LAC_-_Argentina.webp
  cover_position: 54.48
  cover_only: true
- title: Engaging citizens to monitor public projects in Peru
  link: /casos-de-estudio/engaging-citizens-to-monitor-public-projects-in-peru
  cover: /assets/images/LAC_-_Peru.webp
  cover_position: 53.12
  cover_only: true
- title: Engaging citizens to monitor corruption in public construction projects in Colombia
  link: /casos-de-estudio/engaging-citizens-to-monitor-corruption-in-public-construction-projects-in-colombia
  cover: /assets/images/LAC_-_Colombia.webp
  cover_only: true
- title: Promoting SMEs by reserving public contracts in the Dominican Republic
  link: /casos-de-estudio/promoting-smes-by-reserving-public-contracts-in-the-dominican-republic
  cover: /assets/images/LAC_-_Dominican_Republic.webp
  cover_only: true
- title: Establishing SPP governance in an Action Plan in Ecuador
  link: /casos-de-estudio/establishing-spp-governance-in-an-action-plan-in-ecuador
  cover: /assets/images/LAC_-_Ecuador.webp
  cover_only: true
- title: "Using open contracting to procure a sustainable and user-friendly bike share system in Mexico City"
  link: /casos-de-estudio/using-open-contracting-to-procure-a-sustainable-and-user-friendly-bike-share-system-in-mexico-city
  cover: /assets/images/LAC_-_Mexico.webp
  cover_only: true
{% endgallery %}
{% enddatabase %}

{% database Europe %}
{% gallery large %}

- title: Standardizing sustainability requirements according to market capability in the Basque Country
  link: /casos-de-estudio/standardizing-sustainability-requirements-according-to-market-capability-in-the-basque-country
  cover: /assets/images/Europe_-_Basque_Country.webp
  cover_only: true
- title: Creating a knowledge network for procurement officers in The Netherlands
  link: /casos-de-estudio/creating-a-knowledge-network-for-procurement-officers-in-the-netherlands
  cover: /assets/images/Europe_-_The_Netherlands.webp
  cover_only: true
- title: Communicating prioritized procurement categories in an SPP Action Plan in Ireland
  link: /casos-de-estudio/communicating-prioritized-procurement-categories-in-an-spp-action-plan-in-ireland
  cover: /assets/images/Europe_-_Ireland.webp
  cover_only: true
- title: Promoting circularity through furniture procurement in Wales
  link: /casos-de-estudio/promoting-circularity-through-furniture-procurement-in-wales
  cover: /assets/images/Europe_-_Wales.webp
  cover_only: true
- title: Creating a task force to monitor supplier working conditions in the City of Copenhagen
  link: /casos-de-estudio/creating-a-task-force-to-monitor-supplier-working-conditions-in-the-city-of-copenhagen
  cover: /assets/images/Europe_-_Copenhagen.webp
  cover_only: true
- title: Engaging with the market to facilitate supplier collaboration in Scotland
  link: /casos-de-estudio/engaging-with-the-market-to-facilitate-supplier-collaboration-in-scotland
  cover: /assets/images/Europe_-_Scotland.webp
  cover_only: true
- title: Engaging with the market to procure socially responsible ICT in Stockholm
  link: /casos-de-estudio/engaging-with-the-market-to-procure-socially-responsible-ict-in-stockholm
  cover: /assets/images/Europe_-_Stockholm.webp
  cover_only: true
- title: Considering CO2 emissions when selecting suppliers in The Netherlands
  link: /casos-de-estudio/considering-co2-emissions-when-selecting-suppliers-in-the-netherlands
  cover: /assets/images/Europe_-_The_Netherlands_2.webp
  cover_position: 62.89
  cover_only: true
- title: Monitoring progress against sustainability goals in Lithuania
  link: /casos-de-estudio/monitoring-progress-against-sustainability-goals-in-lithuania
  cover: /assets/images/Europe_-_Lithuania.webp
  cover_only: true
- title: Engaging with users to assess procurement needs in Italy
  link: /casos-de-estudio/engaging-with-users-to-assess-procurement-needs-in-italy
  cover: /assets/images/Europe_-_Italy.webp
  cover_only: true
- title: Establishing a network of civil society procurement monitors in Ukraine
  link: /casos-de-estudio/establishing-a-network-of-civil-society-procurement-monitors-in-ukraine
  cover: /assets/images/Europe_-_Ukraine-2.webp
  cover_position: 55.89
  cover_only: true
{% endgallery %}
{% enddatabase %}

{% database East and Southeast Asia %}
{% gallery large %}

- title: Prioritizing procurement categories in Vietnam
  link: /casos-de-estudio/prioritizing-procurement-categories-in-vietnam
  cover: /assets/images/Asia_-_Vietnam-2.webp
  cover_position: 55.01
  cover_only: true
- title: Establishing a system for monitoring SPP implementation and CO2 emissions in Korea
  link: /casos-de-estudio/establishing-a-system-for-monitoring-spp-implementation-and-co2-emissions-in-korea
  cover: /assets/images/Asia_-_Korea.webp
  cover_only: true
- title: Setting up a catalogue of sustainable suppliers in Thailand
  link: /casos-de-estudio/setting-up-a-catalogue-of-sustainable-suppliers-in-thailand
  cover: /assets/images/Asia_-_Thailand.webp
  cover_only: true
{% endgallery %}
{% enddatabase %}

{% database North America %}
{% gallery large %}

- title: Gathering supplier performance data in Wichita
  link: /casos-de-estudio/gathering-supplier-performance-data-in-wichita
  cover: /assets/images/USA_-_Wichita.webp
  cover_position: 52.29
  cover_only: true
{% endgallery %}
{% enddatabase %}
